...
INSERT INTO `AdminSession` VALUES (5,1,'c3b14d421fab47dab9d3de84093f1cb4');
...
CREATE TABLE `NutritionalProfiles` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `NutritionalProfiles_UN` (`recipe_id`),
...
INSERT INTO `Recipes` VALUES (17,'Pasta','Desc','image','ingedients',1,'Italian'),(19,'Pasta with parm','Desc','image','ingedients',0,'Italian');
...
/*!50003 SET collation_connection  = utf8mb4_unicode_ci */ ;
DELIMITER ;;
CREATE DEFINER=`Cameron`@`localhost` PROCEDURE `createInstructions`(idInput int unsigned, prepInput varchar(500), cookInput varchar(500), methodsInput varchar(500),tokenInput varchar(500), adminIdInput int unsigned)
    MODIFIES SQL DATA
BEGIN
	if tokenInput = (select session_token from AdminSession where admin_id = adminIdInput) then
...
/*!50003 SET collation_connection  = utf8mb4_unicode_ci */ ;
DELIMITER ;;
CREATE DEFINER=`Cameron`@`localhost` PROCEDURE `createNutritionalProfile`(recipeIdInput int unsigned, proteinInput float, fatInput float, carbInput float, calInput float, satfatInput float, sugarInput float, saltInput int, tokenInput varchar(500), adminIdInput int unsigned)
    MODIFIES SQL DATA
BEGIN
	if tokenInput = (select session_token from AdminSession where admin_id = adminIdInput) then
...
/*!50003 SET @saved_sql_mode       = @@sql_mode */ ;
/*!50003 SET sql_mode              = 'IGNORE_SPACE,STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
/*!50003 DROP PROCEDURE IF EXISTS `fetchUserKey` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET collation_connection  = utf8mb4_unicode_ci */ ;
DELIMITER ;;
CREATE DEFINER=`Cameron`@`localhost` PROCEDURE `fetchUserKey`(clientIdInput int unsigned, tokenInput varchar(255))
BEGIN
	declare tokenChecker varchar(255);
    set tokenChecker = (select session_token from ClientSession where client_id = clientIdInput);
	if tokenInput = tokenChecker then
	select convert(apiKey using "utf8") as apiKey from UserKeys where user_id = clientIdInput; 
    end if;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET @saved_sql_mode       = @@sql_mode */ ;
/*!50003 SET sql_mode              = 'IGNORE_SPACE,STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
/*!50003 DROP PROCEDURE IF EXISTS `getClientInfo` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET collation_connection  = utf8mb4_unicode_ci */ ;
DELIMITER ;;
CREATE DEFINER=`Cameron`@`localhost` PROCEDURE `getClientInfo`(clientIdInput int unsigned)
BEGIN
	if clientIdInput is not null then
	select convert(username using "utf8") as username,convert(email using "utf8") as email, convert(created_at using "utf8") as created_at from Client where id = clientIdInput;
    end if;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET collation_connection  = utf8mb4_unicode_ci */ ;
DELIMITER ;;
CREATE DEFINER=`Cameron`@`localhost` PROCEDURE `getInstructions`(idInput int unsigned, keyInput varchar(500))
BEGIN
...
/*!50003 SET collation_connection  = utf8mb4_unicode_ci */ ;
DELIMITER ;;
CREATE DEFINER=`Cameron`@`localhost` PROCEDURE `getNutrition`(recipeIdInput int unsigned, keyInput varchar(500))
BEGIN
	declare confirmedKey BOOL;
	declare keyValue varchar(500);
    set keyValue = (select apiKey from UserKeys where apiKey = keyInput);
...
/*!50003 SET collation_connection  = utf8mb4_unicode_ci */ ;
DELIMITER ;;
CREATE DEFINER=`Cameron`@`localhost` PROCEDURE `getRecipeId`(titleInput varchar(255), keyInput varchar(500) )
BEGIN
	declare confirmedKey BOOL;
	declare keyValue varchar(500);
    set keyValue = (select apiKey from UserKeys where apiKey = keyInput);
...
/*!50003 SET collation_connection  = utf8mb4_unicode_ci */ ;
DELIMITER ;;
CREATE DEFINER=`Cameron`@`localhost` PROCEDURE `searchByCuisine`(searchInput varchar(255), keyInput varchar(500), isHealthyInput bool)
BEGIN
	declare confirmedKey BOOL;
	declare keyValue varchar(500);
    set keyValue = (select apiKey from UserKeys where apiKey = keyInput);
    ...
    if keyValue is not null then
    set confirmedKey = 1;
        if confirmedKey = 1 and isHealthyInput is null THEN
           	select id,
	    ...
	    elseif confirmedKey = 1 and isHealthyInput is not null THEN 
           	select id,
	        convert(title using "utf8") as title,
         	convert(`desc` using "utf8") as `desc`,
	        convert(image_url using "utf8") as image_url,
        	isHealthy,
	        convert(cuisine using "utf8") as cuisine 
	        from Recipes where cuisine = searchInput and isHealthy = isHealthyInput;
	    end if;
	end if;
END ;;
DELIMITER ;
...
/*!50003 SET collation_connection  = utf8mb4_unicode_ci */ ;
DELIMITER ;;
CREATE DEFINER=`Cameron`@`localhost` PROCEDURE `searchRecipe`(searchInput varchar(255), keyInput varchar(500), isHealthyInput bool)
BEGIN
	declare confirmedKey BOOL;
	declare keyValue varchar(500);
    set keyValue = (select apiKey from UserKeys where apiKey = keyInput);
    if keyValue is not null then
        ...
        if confirmedKey = 1 and isHealthyInput is null THEN
	     ...
	     elseif confirmedKey = 1 and isHealthyInput is not null THEN 
	      ...
	      isHealthy,
	      convert(cuisine using "utf8") as cuisine 
	      from Recipes where title = searchInput and isHealthy = isHealthyInput;
	     end if;
	end if;
END ;;
DELIMITER ;
...
-- Dump completed on 2023-10-27 16:08:55
