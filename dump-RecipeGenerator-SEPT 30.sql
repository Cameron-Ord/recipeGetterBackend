...
CREATE TABLE `RecipeInstructions` (
  ...
  PRIMARY KEY (`id`),
  KEY `RecipeInstructions_FK` (`recipe_id`),
...
CREATE DEFINER=`Cameron`@`localhost` PROCEDURE `adminLogin`(usernameInput varchar(100), passwordInput varchar(100), tokenInput varchar(255))
    MODIFIES SQL DATA
BEGIN
	declare adminPassword varchar(100);
    declare adminId int unsigned;
    select password into adminPassword from Admin where password = passwordInput;
    select id INTO adminId from Admin where username=usernameInput;
    if adminId is not null and adminPassword is not null then
...
/*!50003 SET collation_connection  = utf8mb4_unicode_ci */ ;
DELIMITER ;;
CREATE DEFINER=`Cameron`@`localhost` PROCEDURE `clientLogin`(usernameInput varchar(100), passwordInput varchar(255), tokenInput varchar(255))
    MODIFIES SQL DATA
BEGIN
	declare clientPassword varchar(100);
    declare clientId int unsigned;
    set clientPassword = (select password from Client where password = passwordInput);
    set clientId = (select id from Client where username=usernameInput);
    if clientId is not null and clientPassword is not null then
	  ...
	  select clientId, convert(tokenInput using "utf8") as tokenInput;
	commit;
	end if;
END ;;
DELIMITER ;
...
/*!50003 SET collation_connection  = utf8mb4_unicode_ci */ ;
DELIMITER ;;
CREATE DEFINER=`Cameron`@`localhost` PROCEDURE `clientSignup`(usernameInput varchar(100), passwordInput varchar(100), emailInput varchar(255), tokenInput varchar(255))
    MODIFIES SQL DATA
BEGIN
    ...
    set confirmedId = (select id from Client where username = usernameInput);
    if confirmedId is not null and tokenInput is not null then
       insert into ClientSession (client_id,session_token) values (confirmedId,tokenInput);
       select confirmedId, convert(tokenInput using "utf8") as tokenInput;
    end if;
   commit;
END ;;
DELIMITER ;
...
/*!50003 SET collation_connection  = utf8mb4_unicode_ci */ ;
DELIMITER ;;
CREATE DEFINER=`Cameron`@`localhost` PROCEDURE `createRecipe`(titleInput varchar(255), descInput varchar(255), imageInput varchar(250), ingredientInput varchar(500), isHealthyInput BOOL, cuisineInput varchar(255))
    MODIFIES SQL DATA
BEGIN
...
/*!50003 SET collation_connection  = utf8mb4_unicode_ci */ ;
DELIMITER ;;
CREATE DEFINER=`Cameron`@`localhost` PROCEDURE `generateKey`(clientIdInput int unsigned, generatedToken varchar(250))
    MODIFIES SQL DATA
BEGIN
...
/*!50003 SET collation_connection  = utf8mb4_unicode_ci */ ;
DELIMITER ;;
CREATE DEFINER=`Cameron`@`localhost` PROCEDURE `searchByCuisine`(searchInput varchar(255))
BEGIN
	select id,
...
-- Dump completed on 2023-09-30 22:26:14
