...
BEGIN
    ...
    if adminId is not null and adminPassword is not null then
	  insert into AdminSession (session_token, admin_id) values (tokenInput,adminId);
	commit;
	else 
	  select row_count();
	end if;
...
/*!50003 SET collation_connection  = utf8mb4_unicode_ci */ ;
DELIMITER ;;
CREATE DEFINER=`Cameron`@`localhost` PROCEDURE `clientLogin`()
    MODIFIES SQL DATA
BEGIN
	declare clientPassword varchar(100);
    declare clientId int unsigned;
    select password into clientPassword from Client where password = passwordInput;
    select id INTO clientId from Client where username=usernameInput;
    if clientId is not null and clientPassword is not null then
	  insert into ClientSession (session_token, client_id) values (tokenInput,clientId);
	commit;
	else 
	  select row_count();
	end if;
...
BEGIN
	...
	from Recipes where cuisine like concat('%', searchInput, '%');
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
BEGIN
	...
	isHealthy,
	convert(cuisine using "utf8") as cuisine 
	from Recipes where title like concat('%', searchInput, '%');
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2023-09-30 15:04:39
